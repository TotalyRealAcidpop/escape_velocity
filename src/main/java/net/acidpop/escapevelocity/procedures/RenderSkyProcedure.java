...
import net.acidpop.escapevelocity.network.EscapeVelocityModVariables;
...
public class RenderSkyProcedure {
	...
	private static final Predicate<Object[]> PREDICATE = params -> {
		...
		if (entity != null) {
			...
			execute(null, level, level.dimension(), entity);
			return true;
		}
		return false;
	};
	...
	private static void execute(@Nullable Event event, LevelAccessor world, ResourceKey<Level> dimension, Entity entity) {
		if (dimension == null || entity == null)
			return;
		if (EscapeVelocityModVariables.WorldVariables.get(world).earthStage >= 28) {
			EscapeVelocityModVariables.WorldVariables.get(world).earthStage = 1;
			EscapeVelocityModVariables.WorldVariables.get(world).syncData(world);
		}
		if (dimension == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("escape_velocity:moon"))) {
			RenderSystem.setShaderTexture(0, new ResourceLocation(("escape_velocity" + ":textures/" + "black_sky" + ".png")));
			renderSkybox(0, 0, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
			renderStars(1500, 10842, 90, 0, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
			EscapeVelocityModVariables.WorldVariables.get(world).earthStage = EscapeVelocityModVariables.WorldVariables.get(world).earthStage + 0.025;
			EscapeVelocityModVariables.WorldVariables.get(world).syncData(world);
			RenderSystem.setShaderTexture(0, new ResourceLocation(("escape_velocity" + ":textures/" + ("earths/earth" + Math.round(EscapeVelocityModVariables.WorldVariables.get(world).earthStage)) + ".png")));
			renderTexture(40, 160, -55, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
			RenderSystem.setShaderTexture(0, new ResourceLocation(("minecraft" + ":textures/" + "environment/sun" + ".png")));
			renderSun(60, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
			if (entity.getY() > 130) {
				RenderSystem.setShaderTexture(0, new ResourceLocation(("escape_velocity" + ":textures/" + "moon" + ".png")));
				renderTexture((float) ((-0.2299) * entity.getY() + 329.9), 0, 90, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
			}
		}
	}
}
