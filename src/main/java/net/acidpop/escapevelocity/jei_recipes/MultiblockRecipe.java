...
public class MultiblockRecipe implements Recipe<SimpleContainer> {
	...
	public MultiblockRecipe(ResourceLocation id, ItemStack output, NonNullList<Ingredient> recipeItems) {
		this.id = id;
		this.output = output;
	...
	public static class Serializer implements RecipeSerializer<MultiblockRecipe> {
		...
		public static final ResourceLocation ID = new ResourceLocation("escape_velocity", "multiblock");
		...
		public MultiblockRecipe fromJson(ResourceLocation pRecipeId, JsonObject pSerializedRecipe) {
			...
			return new MultiblockRecipe(pRecipeId, output, inputs);
		}
		...
		public @Nullable MultiblockRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
			...
			return new MultiblockRecipe(id, output, inputs);
		}
